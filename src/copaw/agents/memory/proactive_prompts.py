...
1. Infer the 1 to 3 larger ongoing tasks the user is most likely trying to complete recently.
2. For each task, identify what information the user is most likely still missing or actively concerned about.
3. Convert that missing information need into one concrete query.
4. Check whether that query would substantially duplicate a command, search, or request already present in memory.
5. If it duplicates prior user activity, replace it with a different but still relevant query that addresses an adjacent unmet information need.
...
- Not a literal one-line command previously typed by the user.
...
- Order by priority.
- Prefer the tasks most likely to matter now.
