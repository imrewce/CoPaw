...
session_references: Dict[str, Any] = {}
...
def enable_proactive_for_session(
    ...
    # Filter out None values if necessary, or just store as is
    session_references[session_id] = {k: v for k, v in refs.items() if v is not None}
...
class ActiveAgentMonitor:
    """Monitor the active agent to detect if it is busy."""
...
async def _handle_proactive_trigger(
    ...
    if not last_interaction_was_before_mode_enabled:
        if await is_last_message_proactive():
    ...
    try:
        refs = session_references.get(session_id, {})
        responder_task = asyncio.create_task(
            generate_proactive_response(
                session_id,
                in_memory=refs.get('in_memory')
            )
        )
    ...
    while True:
        try:
            ...
            last_trigger_attempt = await _handle_proactive_trigger(
                session_id, config, last_trigger_attempt
            )
